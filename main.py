...
# --- LÓGICA DE ASSINATURA (O QUE O FUNCIONÁRIO VÊ) ---
if "confirmar" in st.query_params:
    ...
    st.info("Você já pode fechar esta aba.")
    st.stop()
...
# --- DASHBOARD (COM ALERTAS DE CONFIRMAÇÃO) ---
if menu == "📊 Dashboard":
    st.markdown('<div class="main-header">📊 Monitoramento em Tempo Real</div>', unsafe_allow_html=True)
    
    # Busca as últimas confirmações para mostrar o alerta
    recentes = supabase.table("entregas").select("*, oficiais(nome), ep(nome)").eq("status", "Confirmado ✅").order("id", desc=True).limit(1).execute()
    if recentes.data:
        nome_f = recentes.data[0]['oficiais']['nome']
        epi_f = recentes.data[0]['ep']['nome']
        st.toast(f"✅ Nova confirmação: {nome_f} recebeu {epi_f}!", icon="🛡️")
...
# --- CONFIGURAÇÕES ---
    ...
    st.markdown("### ⚙️ Link do Sistema")
    nova_url = st.text_input("Cole aqui a URL do navegador", url_base)
