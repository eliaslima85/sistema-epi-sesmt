...
def gerar_pdf_ficha(f, df):
    ...
    # CORREÇÃO: RETIRADO "NOME E" DO CNPJ
    ...
    pdf.cell(25, 8, "VALID. C.A.", 1, 0, 'C', fill=True) # VALIDADE AQUI
    ...
    pdf.ln(10)
    # TEXTO LEGAL SOLICITADO (NR 6)
    pdf.set_font("Arial", 'I', 8)
    text_legal = '6.5.1 alinea d da NR 6 "d) registrar o seu fornecimento ao empregado, podendo ser adotados livros, fichas ou sistema eletronico, inclusive, por sistema biometrico;"'
    pdf.multi_cell(0, 5, text_legal, align='J')
...
if "confirmar" in st.query_params:
    ...
    st.balloons()
    st.success("🛡️ RECEBIMENTO CONFIRMADO! O SESMT HUC já recebeu sua assinatura.")
    st.stop()
...
menu = st.sidebar.radio("SESMT MENU", ["📊 Dashboard", "🚀 Entregar EPI", "👥 Funcionários", "📦 Catálogo", "📄 Ficha de EPI", "⚙️ Configurações"])
...
if menu == "📊 Dashboard":
    ...
    if res.data:
        for row in res.data:
            ...
            col1.write(f"{status_cor} **{row['oficiais']['nome']}** - {row['ep']['nome']} - {row['status']}")
            if "Pendente" in row['status']:
                link = f"{url_base}/?confirmar={row['token']}"
                msg = urllib.parse.quote(f"🛡️ *SESMT HUC*\nFavor assinar o recebimento: {row['ep']['nome']}\nLink: {link}")
...
# --- CATÁLOGO (CADASTRAR COM VALIDADE) ---
    ...
    with t1:
        with st.form("cad_epi", clear_on_submit=True):
            ...
            ca_epi = st.text_input("Numero do C.A.")
            val_epi = st.date_input("Data de Validade do C.A.")
            if st.form_submit_button("Salvar EPI"):
                supabase.table("ep").insert({"nome": n_epi, "ca": ca_epi, "validade": str(val_epi)}).execute()
                st.success("EPI cadastrado com validade!"); st.rerun()
    with t2:
...
# --- FICHA DE EPI ---
    ...
    if not df_f.empty:
        ...
        if h_res.data:
            ...
            if st.button("📥 Gerar FICHA DE ENTREGA DE EPI - NR 06"):
                st.download_button("Clique aqui para Baixar", gerar_pdf_ficha(f_d, df_h), f"Ficha_{f_d['matricula']}.pdf")
...
# --- ENTREGAR EPI ---
    ...
    if not df_f.empty and not df_e.empty:
        colab = st.selectbox("Escolha o Colaborador", df_f['matricula'] + " - " + df_f['nome'])
        df_e['disp'] = df_e['nome'].astype(str) + " (CA: " + df_e['ca'].astype(str) + ")"
        sel_epis = st.multiselect("Selecione os Itens", options=df_e['disp'])
        if st.button("Gerar Link e Enviar Zap"):
            if sel_epis:
                tk = str(random.randint(100000, 999999))
                f_d = df_f[df_f['matricula'] == colab.split(" - ")[0]].iloc[0]
                for item in sel_epis:
                ...
                msg = urllib.parse.quote(f"🛡️ *SESMT HUC*\nAssine o recebimento do EPI: {', '.join(sel_epis)}\nLink: {link}")
...
# --- CONFIGURAÇÕES ---
    ...
    nova = st.text_input("URL Pública do App", url_base)
    if st.button("Salvar URL"):
        supabase.table("oficiais").upsert({"matricula": "URL_SISTEMA", "whatsapp": nova}).execute()
