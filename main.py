...
if menu == "📊 Dashboard":
    ...
    if res.data:
        for row in res.data:
            ...
            c1.write(f"{status_cor} **{row['oficiais']['nome']}** - {row['ep']['nome']}")
...
# --- ENTREGAR EPI (CORRIGIDO PARA SUPABASE) ---
elif menu == "🚀 Entregar EPI":
    st.markdown("### 🚀 Registrar Nova Entrega")
    ...
    if not df_f.empty and not df_e.empty:
        ...
        epis_sel = st.multiselect("EPIs", df_e['nome'] + " (CA: " + df_e['ca'] + ")")
        
        if st.button("Confirmar e Gerar Link"):
            if epis_sel:
                ...
                msg = urllib.parse.quote(f"🛡️ *SESMT HUC*\nRecebimento de EPI: {', '.join(epis_sel)}\nLink para assinar: {link}")
                st.markdown(f'<a href="https://api.whatsapp.com/send?phone=55{f_d["whatsapp"]}&text={msg}" target="_blank"><button style="width:100%; background-color:#25D366; color:white; border:none; padding:15px; border-radius:5px; font-weight:bold; cursor:pointer;">📲 ENVIAR AGORA PELO WHATSAPP</button></a>', unsafe_allow_html=True)
...
# --- CONFIGURAÇÕES ---
    ...
    st.markdown("### ⚙️ Configurar URL do WhatsApp")
    nova_url = st.text_input("Cole aqui a URL do seu site (da barra do navegador)", url_base)
    if st.button("Salvar Configuração"):
        supabase.table("oficiais").upsert({"matricula": "URL_SISTEMA", "whatsapp": nova_url}).execute()
        st.success("URL Salva! Agora o WhatsApp vai funcionar.")
